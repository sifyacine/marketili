...
const ROLES = [
  ...
  { id:"freelancer", icon:"⚡", labelFR:"Freelancer / Influenceur", descFR:"Je propose mes compétences créatives" },
];
...
const Register = () => {
  ...
  const handleSubmit = async e => {
    ...
    if (formData.password.length < 8) return setError("Minimum 8 caractères");
    ...
    <div className="auth-page">
      <div className="auth-left">
        ...
        <div className="auth-left-steps">
          {[{n:"01",t:"Créez votre compte"},{n:"02",t:"Explorez les opportunités"},{n:"03",t:"Lancez vos collaborations"}].map(s=>(
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="s1" variants={slide} initial="hidden" animate="visible" exit="exit">
                ...
                <div className="role-select-grid">
                  {ROLES.map(r => (
                    <button key={r.id} className="role-select-card" type="button"
            ...
            {step === 2 && (
              <motion.div key="s2" variants={slide} initial="hidden" animate="visible" exit="exit">
                ...
                <div className="account-type-grid">
                  {[{id:"person",icon:"👤",name:"Personne",desc:"Compte personnel"},{id:"company",icon:"🏭",name:"Entreprise",desc:"Compte professionnel"}].map(t=>(
                    <button key={t.id} className="account-type-card" type="button"
            ...
            {step === 3 && (
              <motion.div key="s3" variants={slide} initial="hidden" animate="visible" exit="exit">
                <button className="auth-back-btn" type="button" onClick={() => setStep(role==="client"?2:1)}>← Retour</button>
                <div className="auth-form-header">
                ...
                <form onSubmit={handleSubmit} className="auth-form">
                  {(FIELDS[fieldKey]||[]).map(f => (
                    <div className="form-group" key={f.name}>
                      ...
                      <input className="form-input" type={f.type} name={f.name}
                        required={f.required} value={formData[f.name]||""} onChange={handleChange} />
                    </div>
                  ))}
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="email" name="email" required
                      placeholder="vous@exemple.com" value={formData.email} onChange={handleChange} />
                  </div>
                  <div className="form-row-2">
                    <div className="form-group">
                      <label className="form-label">Mot de passe</label>
                      <input className="form-input" type="password" name="password" required
                        placeholder="Min. 8 caractères" value={formData.password} onChange={handleChange} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Confirmer</label>
                      <input className="form-input" type="password" name="confirmPassword" required
                        placeholder="Répétez" value={formData.confirmPassword} onChange={handleChange} />
                    </div>
                  </div>
