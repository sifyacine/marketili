  ...
  try {
    ...
    if (Array.isArray(assignedTo) && assignedTo.length > 0) {
      assignedTo.forEach(a => {
        ...
        notifiedIds.add(String(a.memberId));
      });
    }

    // Notify other project members who weren't directly assigned to this task
    (project.assignedMembers || []).forEach(m => {
      if (notifiedIds.has(String(m.memberId))) return;
      const isTeam = m.memberType === "TeamMember";
      const mRole  = isTeam ? "team_member"  : "agency_member";
      const mModel = isTeam ? "TeamMember"   : "AgencyMember";
      const mPath  = isTeam ? "team"          : "agency";
      Notification.notify({
        recipient: m.memberId, recipientRole: mRole, recipientModel: mModel,
        type: "project_milestone", category: "tasks",
        title: `Nouvelle tâche : ${title}`,
        body: `Une nouvelle tâche "${title}" a été ajoutée au projet "${project.title}".`,
        link: `/dashboard/${mPath}/tasks`,
        metadata: { projectId: project._id },
      });
    });
    ...
    if (project.providerAgency) {
      Notification.notify({
        ...
        type: "project_milestone", category: "tasks",
        title: `Nouvelle tâche : ${title}`,
        body: `Une nouvelle tâche a été créée dans le projet "${project.title}".`,
